...
def main(wenji_list, start, end, kw):
    ...
    for wenji in wenji_list:
        new_header = deepcopy(DEFAULT_HEADERS)
        new_header.update({'Referer': BASE_URL.format(wenji)})

        # 以文集的数字作为子文件名
        save_path = os.path.join(SCRIPT_CONFIG['SAVE_PATH'], str(wenji))

        if kw.get('order_by') == 'comment':
            order_by = ORDER_COMMENT
        elif kw.get('order_by') == 'add':
            order_by = ORDER_ADD
        elif kw.get('order_by') == 'seq':
            order_by = ORDER_SEQ
        else:
            # 默认seq
            order_by = ORDER_SEQ
        ...
        task = Task.make_task({
            'url': API_URL.format(wenji, order_by, start),
            'method': 'GET',
...
def parser_list(task):
    ...
    with ArticleDB(task['save']['save_path']) as article_db:
        article_db.insert_meta_data(['BOOK_NAME', format_file_name('简书文集_' + book_name)], update=False)
    ...
    for item in items:
        ...
        try:
            ...
            LOG.log_it('解析文集标题出错（如一直出现，而且浏览器能正常访问，可能是网站网站代码升级，请通知开发者。）', 'WARN')
            raise RetryDownload
